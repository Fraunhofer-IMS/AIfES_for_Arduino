/*
  ...
  The calculation is done in float 32 (F32).
  ...
  For this, AIFES_WITH_CMSIS must be activated in aifes_config.h and CMSIS-DSP library must be available inside the AIfES_for_Arduino library.
...
void print_tictactoe_board(float *board)
{
  uint8_t i, j;
...
void loop() {
  ...
  for(i = 0; i < 5; i++){
    ...
    Serial.print(F("\nYour turn was ")); Serial.print(user_input[0]); Serial.println(user_input[1]);
    ...
    Serial.print(F("\nThe AI took ")); Serial.print(timer); Serial.println(F(" μs to think about the turn."));
